/**
 * In-memory per-chat conversation session manager.
 *
 * Each chatId maps to a `Session` containing the running message history
 * and the timestamp of the last activity. Sessions older than `ttlMinutes`
 * are evicted by a periodic cleanup interval.
 *
 * Messages are stored in the Vercel AI SDK v4 `CoreMessage` format so that
 * they can be passed directly to `streamText`/`generateText`.
 */
...
export class SessionManager {
  ...
  constructor(ttlMinutes: number) {
    if (!Number.isFinite(ttlMinutes) || ttlMinutes <= 0) {
      throw new Error(`SessionManager: ttlMinutes must be > 0 (got ${ttlMinutes}).`);
    }
